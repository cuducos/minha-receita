...
const (
	...
	limit       = uint16(1000) // max results per paginated search
)
...
func (app *api) messageResponse(w http.ResponseWriter, s int, m string) {
	...
	if err != nil {
		...
		app.errorLogger.Write(fmt.Appendf(nil, "Could not wrap message in JSON: %s", m))
		return
	}
...
func (app *api) companyHandler(w http.ResponseWriter, r *http.Request) {
	...
	pth := r.URL.Path
	if pth == "/" {
		q := db.NewQuery(r.URL.Query(), limit)
		if q == nil {
