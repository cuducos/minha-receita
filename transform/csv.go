...
func (a *dataset) lineProducer(l chan<- []string, f *zip.File) error {
	z, err := f.Open()
	if err != nil {
		return err
	}
...
func (a *dataset) lineConsumer(l chan []string) {
	for {
		s, ok := <-l
		if !ok {
			return
...
func headersFor(a *dataset) ([]string, error) {
	var h []string
	switch a.kind {
...
func (a *dataset) createCsv() error {
	p := a.csvPath()
...
func (a *dataset) writeCsv(q chan<- error) {
	ls, err := a.files()
	if err != nil {
		q <- err
		return
