...
func TestNewCompany(t *testing.T) {
	...
	srcs := sources()
	if err := kv.put(srcs["mot"], "00", []string{"SEM MOTIVO"}); err != nil {
		t.Fatalf("expected no error writing to kv, got %s", err)
	}
	...
	if got.DescricaoMotivoSituacaoCadastral != nil {
		t.Errorf("expected DescricaoMotivoSituacaoCadastral to be nil, got %v", got.DescricaoMotivoSituacaoCadastral)
	}
	...
	if *got.CNAEFiscalDescricao != "TRATAMENTO DE DADOS, PROVEDORES DE SERVIÇOS DE APLICAÇÃO E SERVIÇOS DE HOSPEDAGEM NA INTERNET" {
		t.Errorf("expected CNAEFiscalDescricao to be 'TRATAMENTO DE DADOS, PROVEDORES DE SERVIÇOS DE APLICAÇÃO E SERVIÇOS DE HOSPEDAGEM NA INTERNET', got %s", *got.CNAEFiscalDescricao)
	}
	...
	if *got.Municipio != "BRASÍLIA" {
		t.Errorf("expected Municipio to be BRASÍLIA, got %s", *got.Municipio)
	}
	if got.Telefone1 != "" {
	...
	if got.Email != nil {
		t.Errorf("expected Email to be nil, got %v", got.Email)
	}
	...
	if got.RazaoSocial != "REGIONAL BRASILIA-DF" {
		t.Errorf("expected RazaoSocial to be REGIONAL BRASILIA-DF, got %s", got.RazaoSocial)
	}
	if *got.CodigoNaturezaJuridica != 2046 {
		t.Errorf("expected CodigoNaturezaJuridica to be 2046, got %d", *got.CodigoNaturezaJuridica)
	}
	if *got.NaturezaJuridica != "SOCIEDADE EMPRESÁRIA LIMITADA" {
		t.Errorf("expected NaturezaJuridica to be SOCIEDADE EMPRESÁRIA LIMITADA, got %s", *got.NaturezaJuridica)
	}
	if *got.QualificacaoDoResponsavel != 10 {
		t.Errorf("expected QualificacaoDoResponsavel to be 10, got %d", *got.QualificacaoDoResponsavel)
	}
	if *got.CapitalSocial != 100000.0 {
		t.Errorf("expected CapitalSocial to be 100000.0, got %f", *got.CapitalSocial)
	}
	if *got.CodigoPorte != 3 {
		t.Errorf("expected CodigoPorte to be 3, got %d", *got.CodigoPorte)
	}
	if *got.Porte != "DEMAIS" {
	...
	if len(got.QuadroSocietario) != 5 {
		t.Errorf("expected QuadroSocietario to have 5 items, got %d items", len(got.QuadroSocietario))
	}
	...
	if *got.DataOpcaoPeloSimples != date(time.Date(2007, 7, 1, 0, 0, 0, 0, time.UTC)) {
		t.Errorf("expected DataOpcaoPeloSimples to be 2007-07-01, got %v", *got.DataOpcaoPeloSimples)
	}
	...
	if len(got.QuadroSocietario) != 5 {
		t.Errorf("expected QuadroSocietario to have 5 items, got %d", len(got.QuadroSocietario))
	}
	if got.QuadroSocietario[0].NomeSocio != "ANDRE DE CESERO" {
		t.Errorf("expected first partner name to be ANDRE DE CESERO, got %s", got.QuadroSocietario[0].NomeSocio)
	}
	...
	if *got.QuadroSocietario[0].CodigoQualificacaoSocio != 10 {
		t.Errorf("expected first partner qualification code to be 10, got %d", *got.QuadroSocietario[0].CodigoQualificacaoSocio)
	}
	if *got.QuadroSocietario[0].QualificaoSocio != "Diretor" {
		t.Errorf("expected first partner qualification to be Diretor, got %s", *got.QuadroSocietario[0].QualificaoSocio)
	}
	if *got.QuadroSocietario[0].CodigoFaixaEtaria != 6 {
		t.Errorf("expected first partner age range code to be 6, got %d", *got.QuadroSocietario[0].CodigoFaixaEtaria)
	}
	if *got.QuadroSocietario[0].FaixaEtaria != "Entre 51 a 60 anos" {
		t.Errorf("expected first partner age range to be 'Entre 51 a 60 anos', got %s", *got.QuadroSocietario[0].FaixaEtaria)
	}
	if len(got.RegimeTributario) != 1 {
...
func TestNewCompanyWithPrivacy(t *testing.T) {
	...
	got, err := newCompany(sources(), kv, row)
	if err != nil {
		t.Fatalf("expected no error creating a company, got %s", err)
	}
	got.withPrivacy()
	...
	if got.DescricaoTipoDeLogradouro != "" {
		t.Errorf("expected DescricaoTipoDeLogradouro to be empty after privacy for individual, got %s", got.DescricaoTipoDeLogradouro)
	}
	if got.Logradouro != "" {
		t.Errorf("expected Logradouro to be empty after privacy for individual, got %s", got.Logradouro)
	}
	if got.Numero != "" {
		t.Errorf("expected Numero to be empty after privacy for individual, got %s", got.Numero)
	}
	if got.Complemento != "" {
		t.Errorf("expected Complemento to be empty after privacy for individual, got %s", got.Complemento)
	}
	if got.Telefone1 != "" {
		t.Errorf("expected Telefone1 to be empty after privacy for individual, got %s", got.Telefone1)
	}
	if got.Telefone2 != "" {
		t.Errorf("expected Telefone2 to be empty after privacy for individual, got %s", got.Telefone2)
	}
	if got.Fax != "" {
		t.Errorf("expected Fax to be empty after privacy for individual, got %s", got.Fax)
	}
