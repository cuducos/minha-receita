...
func TestMongoDB(t *testing.T) {
	...
	mdb, err := NewMongoDB("minhareceita")
	if err != nil {
	...
	if err := mdb.MetaSave("answer", "42"); err != nil {
		t.Errorf("expected no error writing to the metadata table, got %s", err)
	}
	metadata, err := mdb.MetaRead("answer")
	if err != nil {
	...
	if err := mdb.MetaSave("answer", "forty-two"); err != nil {
		t.Errorf("expected no error re-writing to the metadata table, got %s", err)
	}
	metadata2, err := mdb.MetaRead("answer")
	if err != nil {
